class BillsController < ApplicationController
  ...
  def destroy
    ...
    if !@bill_history.blank?
      ...
      @bills = Bill.where("month = ? and year = ? and customer_id IN(?)", @bill_history.month, @bill_history.year, cus_id)
      @bills.each do |bill|
        bill.destroy
      end if !@bills.blank?
  ...
  def search
    ...
    @bills = Bill.where("paper_id = ? and month = ? and year = ? and is_paid = ?", params[:bill][:paper_id], params[:bill][:month], params[:bill][:year], params[:bill][:is_paid]).page(params[:page]).per_page(1000)
  end
  ...
  def print
    ...
    if not params[:employee_id].blank?
      @customers = Customer.where("employee_id = ?", params[:employee_id])
    else
      @customers = Customer.order("custid ASC")
    end
  ...
  def bill_history_params
    params.fetch(:bill_history, {}).permit(:month, :year, :employee_id)
  end
end
